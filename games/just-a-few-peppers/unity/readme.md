...
The [1_07 physical pepper delivery](../docs/development/tasks/1_07_physical-pepper-batch-comparison.md#delivery-record--september-6-2026) owns the current build, checklist, simulation comparison and verification. The scene supports single/bulk peppers, the complete food loop and four movable props beside the worktop. Normal play has no persistent controls list or instruction footer. **F1** opens all keys, debug tools and optional work help; F1/Esc closes it to the previous mode. Esc alone opens a compact pause menu with session-only sensitivity (0.25x to 2.50x, default 1.00x). Broader settings/options are already planned in 7_02 and 7_03.
...
Aim at a highlighted pepper and **right click** to pick up that one pepper, including exposed crate contents. **E** puts it onto a reachable surface or into the raw crate. Hold/release left mouse with one pepper to charge/throw it. Individual handling is optional.

For bulk work, **right click the orange crate**, then aim at the scattered peppers. **Hold left mouse** to gather the highlighted set into that crate; release stops. The preview selects up to three visible peppers and shrinks to the remaining capacity. At the broad processor tray, **E** pours the amount that fits; real pepper contacts load the intake before automatic processing starts. **F** also pours toward your aim; away from the intake it spills onto the ground. Unaccepted food stays in the crate or recoverable on the ground. Jars accumulate, including partial loads.

**Right click again releases the object from your hand with physics**; **G** does the same. Moving releases retain a little carry motion. Aim near the ground to lower an object before releasing. **E** offers secondary careful placement on ground, worktops or broad stable supports; optional **Z/X** rotates carriers/props. Placement indicators stay hidden, and only a rejected careful-placement attempt explains the obstruction. Regrab with right click. Raw peppers can spill from a toppled crate; gather them again or use R. Finished food stays in its carrier.
...
**R** returns you to the gate, recovers held/lost props and carriers and regroups stray raw peppers at their original supply with all food/progress kept. Valid supported prop/carrier placements stay where you left them. **F8 / Restart food test (clears stored food too)** restores the raw supply and empties both carriers, station and stored food; valid prop arrangements remain. **F9** switches between the default physical batch and the grouped-rest comparison, keeping the same food; see the active mode in F1. Pause/focus freezes motion and processing; release handling controls before acting again after resume. Disk saving, Coins/purchases and direct machine operation remain later tasks. The [queue](../docs/development/tasks/readme.md) owns current progress; use the [fresh-chat prompt](../docs/development/new-chat-prompt.md) to request NEXT.
