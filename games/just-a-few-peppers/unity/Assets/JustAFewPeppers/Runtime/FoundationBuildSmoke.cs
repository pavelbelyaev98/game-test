    ...
    public sealed partial class FoundationBuildSmoke : MonoBehaviour
    {
        ...
        IEnumerator Start()
        {
            ...
            if (session.handling.peppers != null)
            {
                ...
                finished = true;
                File.WriteAllText(Path.Combine(output, "result.txt"), "PASS: Ordinary Windows player; menu/F1/sensitivity, movement/sprint/jump, pause/focus/recovery; retained four-prop physics/stack/throw checks. Physical peppers: exact single pickup and exposed contents, previewed capacity-limited bulk, partial/full physical pours and intake contacts, off-target spills, pause/fresh input, same-ID recovery, both representations, and all 107 units through nine food handoffs. Direct mouse rack/guide motion, release/reverse, focus cancellation, fresh resume and recovery before/after commit pass. Nine measured full/partial cycles use real walking, E pouring, E+W rack operation, E+D grouping and unaccelerated processing. Public gathering prepares each load.\n" + propObservation + "Machine baseline: machine-cycle-baseline.csv and machine-cycle-context.txt. Human machine/grouping feel remains untested. Comparison: pepper-comparison.csv records matched fixed-step physics and handling CPU/managed allocation samples; these are not rendered FPS. Human individual/bulk preference, control clarity, sound and OS focus remain untested.\n");
