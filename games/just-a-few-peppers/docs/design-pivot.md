...
## Physical handling and Cash Cleaner feedback — September 6, 2026

The developer's supplied feedback clarifies the current design without restarting it. The developer enjoyed deliberately picking individual objects, efficiently gathering several, turning loose material into neat bundles/containers, buying useful equipment and selecting what a machine handles in Cash Cleaner. Confusing controls, unclear multi-grab and the phone interface were disliked. These are firsthand preferences, not implementation findings or playtest acceptance for Just a few peppers.

The [mechanics](core-loop-and-mechanics.md#gathering-and-pouring-a-batch) now require both optional deliberate single-pepper pickup and predictable bulk pickup, with a representative scattered group of physical gameplay peppers that can be moved, placed into containers and poured. The [controls](core-loop-and-mechanics.md#single-bulk-placement-and-pouring-controls) require an affected-set preview before bulk action, occlusion and contents/container priority, clear placement/pouring distinctions and brief teaching as actions arrive. Quiet full/invalid feedback and the removed placement indicator remain preserved. The current design is about handling untidy harvest into orderly food, beyond a shrinking decorative pile.

Existing unfinished 1_07 owns this physical sample and measured simulation/representation choice; grouped resting/distant material remains useful, but cannot replace all nearby handling with particles/counts. 1_08 owns one small grouping action within existing finished-output collection, replacing passive receiving from the completed 1_04 baseline. The **provisional candidate** is a directly moved receiving-tray guide that gathers prepared material into a neat batch in the sole carrier. Exact gesture, binding and tuning require testing; detailed filling/capping remains compressed. No per-jar minigame, lid shopping, peeling or extra errands are authorized.

The [state contract](development/state-and-saving.md) tracks live pepper identity/membership, recoverable spills and grouping within the existing output owner; interruption, partial transfers, recovery and later saving preserve the same food without duplicate collection or handoff credit. The existing M3 briefs cover reconstruction. [Prototype validation](scope-and-validation.md#next-experiment-one-pile-one-carrier-one-discovery), 2_03 and 5_05 assess individual handling, bulk handling, packaging and control clarity independently, alongside food appeal and useful purchases. Later art/input/reliability briefs preserve those tested interactions.

Already covered and retained: one handoff-earned Coins balance, separate cumulative stored food, two useful prototype purchases at one bench, one short attachment installation, accessible supplies/short routes, Bulgarian household character, absurd machinery and restrained humor. [Machine selection](scope-and-validation.md#deferred-machine-selection) is only a later candidate: portion/batch-size selection could keep one food type; actual pepper classification adds rules and needs separate approval. Neither is automatically added. Purchases stay at the bench and any later approved settings on the machine; no phone, business, restoration, cellar sorting, stress or compulsory day ending.

This is documentation-only: stable task IDs, completed delivery records, existing workspace implementation and queue delivery/feedback states are preserved. No new implementation or playtest success is claimed. The earlier records below retain their historical automatic-packing and representation decisions; this refinement supersedes those restrictions in the current contracts and unfinished briefs.

Documentation verification: UTF-8, fences, 791 local links and 297 heading targets pass across 59 game/repository-guidance Markdown files; whitespace checks pass. Comparison with the pre-edit workspace preserves all 34 queue rows, nine prototype question IDs, completed task/decision histories and 232 protected implementation/other files. No Unity tests, build or human playtest ran for this documentation update.
