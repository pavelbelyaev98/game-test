...
Status: foundation and crate-handling checks are installed; processing, complete-loop, and save coverage remain planned. [Implementation status](status.md) records delivered results.
...
Task 1_01 installed Unity Test Framework 1.8.0 and separate runtime/editor/EditMode/PlayMode assemblies, with Input System 1.20.0 and uGUI 2.6.0 on Unity 6000.6.0f1. The saved scene and task 1_02's local gathering, conserved quantities, crate ownership, input interruption, and recovery use the same commands below. Processing, save, and complete-loop coverage remain future work.
