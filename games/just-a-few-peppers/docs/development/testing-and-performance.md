...
The packaged smoke also manipulates all four props, then runs the current food-loop checks. The original 1_06 timeout was 210 seconds; 1_07 extends it to 360 seconds for both pepper candidates and repeated handoffs. An isolated five-second sample records the six portable bodies (separate from pepper bodies) and observed batch-update intervals with hardware/configuration in `Logs/FoundationSmoke/loose-props-observation.txt`; captures and disk writes are outside that interval. Batch mode can skip rendering: this sample does not measure rendered FPS, human navigation, minimum-hardware acceptance or a full performance profile. Automation stays muted.

### Physical-pepper checks

`PepperStateTests` covers exact ID ownership, repeated/capacity-limited preview commits, partial acceptance, interrupted/off-target pours, recovery, representation-independent conservation and complete 1/25/107-unit jobs. `PepperBatchSceneTests` uses actual input for intended single pickup, taking exposed contents without grabbing the crate, physical insertion/contact, capped bulk selection, occlusion, full pours, spills, pause/fresh input, recovery and both representations. Shared handling/processing tests now use physical pepper targets and contact timing; toppled crates may spill registered units. The retired mound is no longer a jump platform, so movement coverage uses the existing worktop. Asset checks cover the prefab, 107 seeds and hollow crate walls.

`PepperBatchBuildSmoke` exercises both candidates through actual single pickup, exposed contents, capacity-capped highlighted bulk transfer and intake contact. It checks spills, pause/recovery and completes the 107-unit job through nine unaccelerated processing/receiving/handoff cycles. Public gathering commands prepare full-job loads; the dedicated bulk cases exercise actual LMB input. It retains the menu/movement/comfort and loose-prop probes. Earlier cosmetic raw-food probe branches are not used when `PepperBatch` is present. This is automated input at deterministic approach points, not a human route, full interactive presentation pass or acceptance of feel.

The same player writes `pepper-comparison.csv` and `pepper-comparison-context.txt` beside its captures. Three trials alternate candidate order with equal 107-unit supply and 12-unit filling/pouring setup. Each phase runs 200 fixed 0.02-second physics steps; Stopwatch measures `Physics.Simulate` and `PepperBatch.Tick` separately and thread allocation deltas bracket only those loops. Other scene bodies participate in physics; game time and other Update functions do not advance in these synchronous loops. Captures/file writes are outside measurement. Record active/awake/sleeping counts and CPU/allocation limits; do not call these rendered frame times, full-frame allocations or minimum-hardware evidence. Ordinary input/contact checks run separately with normal game time.

`AuthorPepperBatch` is the guarded initial editor migration; `ConfigurePepperComparison` adds the F9 reference and retires mound targeting, `ScatterPeppers` authors the deterministic scattered layout while preserving source identities, and `AlignPepperCrate` aligns visible crate geometry to its hollow colliders. These are authoring utilities, never routine build prerequisites. `-TestFilter` can select a focused test when investigating a failure. The [1_07 delivery record](tasks/1_07_physical-pepper-batch-comparison.md#delivery-record--september-6-2026) owns current results, artifact and measured decision.
...
| --- | --- | --- | --- |
| A single pepper placed with E hit the raw crate rim and escaped. | The surface hit was used literally even when aiming at the intended container. | `PepperBatchSceneTests` and both packaged candidate checks insert, observe contact and pick the same ID back from exposed contents. | A reachable crate hit selects a forgiving pose above its opening; capacity is checked before release and actual contact follows. |
| A packaged contents capture showed an empty-looking crate despite successful targeting. | The old visible floor was higher than the new physical floor; the pepper sat inside its mesh. Collider-only target checks could not detect this. | The strengthened single-contents PlayMode check failed against the old mesh, requiring the settled pepper skin above the visible floor; both packaged candidates use the same assertion plus inspected captures. | Align the existing floor/wall meshes and marker to the hollow collision geometry through editor authoring. |
| Releasing a loaded crate left its physical contents at the old held pose. | Crate placement updated its transform before cargo changed from held slots to dynamic bodies. | Existing loaded ground/worktop/support checks and toppling/recovery regression exercise the same registered cargo after release. | Carry the cargo to the chosen crate pose before enabling its physics, inheriting bounded crate velocity. Real escaped peppers become recoverable loose owners. |
