...
- Connect each offer to a visible changed part and a plain workflow effect the player can explain after using it. A wide hopper/receiving-tray example may illustrate an existing capacity or handling benefit; it is not permission to add extra offers. Preserve deliberate single pickup, predictable bulk pickup, useful pouring and the tested small output-grouping action while removing redundant handling/confirmations. Detailed filling/capping stays compressed. Purchases remain at the bench, without a phone or automatically added machine-selection settings.
