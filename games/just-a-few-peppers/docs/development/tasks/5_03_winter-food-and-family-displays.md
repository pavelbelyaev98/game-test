...
- Replace 1_04's nearby graybox food group with representative jars whose volume/fill follows each accepted handoff, including between those four milestones. Keep the accumulation readable from the ordinary machine/handoff area. Integrate nearby group, shelves and boxes into the same visual budget; bound/reuse views and derive them from stored units without a second inventory or per-jar physics.
...
- Several deposits within one milestone band, including a small partial deposit, have visible nearby growth/fill feedback from the workstation. Milestone composition changes preserve readable accumulation; neither spending nor an empty/repeated handoff reduces/adds food. Load restores the amount directly without replaying deposits.
...
Deposit several times within one display band, then across thresholds. Inspect the growing supply from the work area, buy an improvement and reload. Stock still reflects all stored food while Coins reflect spending. Report whether you noticed each batch's contribution and cared about the result; visiting the cellar is optional.
