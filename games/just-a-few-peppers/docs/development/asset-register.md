...
Task 1_06 adds four temporary primitive props: hollow basin, four-legged stool, empty crate and sphere ball. Three local graybox materials (`Basin blue`, `Loose prop wood`, `Ball ochre`) and `Ball contact.physicMaterial` accompany them; food carriers' contact material and the existing CC0 plank impact are reused. Each prop has one reusable Rigidbody with matching primitive compound/sphere collision. No external pack, purchase, new license or production-art claim is introduced.

The 1_06 handling revision adds one fixed sloped graybox board using the existing `Loose prop wood` material and a primitive box collider. It adds no body, imported asset or license; the six portable bodies and all earlier materials/audio remain.
