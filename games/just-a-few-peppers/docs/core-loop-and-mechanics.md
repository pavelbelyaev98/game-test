...
E at the intake takes priority over careful placement. G takes priority over a simultaneous E; an active tip presentation finishes or is interrupted before another handling command. Pause/focus/recovery requires releasing scoop, E, G and rotation input before a fresh action. R returns the player to the gate and recovers a held, falling or inaccessible crate to a clear last safe pose or nearby fallback, keeping load, depletion and all station food/progress. A valid supported arrangement stays in place. Out-of-bounds crates recover automatically. F8 or **Restart processing test (clears food)** restores the initial harvest and empties crate/station, returning the crate to its original location.
