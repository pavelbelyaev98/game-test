...
Pause/focus loss freezes a jump and released-object motion in place; explicit resume continues them without a physics burst. Discard pending jump/release requests on pause and reset, and require fresh input after menu activation/resume/reset. Returning to the gate clears vertical motion and jump timing. Retain a steady camera without sprint FOV changes, head bob, or landing shake. Tunable movement values live on the scene's player component. Task 1_05 adds a session-only mouse-sensitivity slider. Its feedback revision moves full controls, debugging keys and current-work help behind F1; normal play has no continuous instruction footer. Esc keeps the compact pause menu. M3 saves currently implemented settings; broader FOV/inversion and rebinding options remain 7_02. Carrier tasks provide free placement while preserving food quantities. A physically reachable shortcut is valid; yard bounds and actual occlusion replace invisible route-order gates.
...
- Keep general control lists and debugging instructions in the optional F1 reference; later options may configure guidance. Retain only brief relevant target/action feedback during normal play.
