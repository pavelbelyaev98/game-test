...
For each changed appearance, call `bake(channel)` for `BaseColor`, `Normal`, `Masks`, then `export()` and `save_batch()`, followed by the validator's `validate()`. Use `preview(view, baked=True)` / `render_lineup()` for temporary inspection only and clean those renders afterward. Sync changed dimensions/triangle expectations in `catalog.json`, `manifest.json` and validation when deliberately changing their contract; visually inspect the result before accepting new expectations.
...
## Retired surface dressing source

135 removes decorative surface stones and their runtime import/renderer/setup.
`export_surface_stones.py`, `SurfaceStones.blend` and `SurfaceStones.fbx` remain
inactive editable Blender source/export derivatives from 134. The approved
collectible meshes, materials, catalog, collision and saved identities remain
in use; B/C material instancing flags return to their original disabled state.
