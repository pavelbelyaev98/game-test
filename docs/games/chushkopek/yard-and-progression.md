...
```mermaid
flowchart LR
    A[A. Gate pocket] --> B[B. Vine lane / wheelbarrow]
    A --> C[C. Shed approach / grinder]
    A --> D[D. Cellar apron / storage shortcut]
    C --> E[E. Rear tarp]
    D --> E
    A --- P[Outdoor processing terrace]
    B --- P
    C --- P
    D --- S[Cellar shelf bays]
    A --- F[Returned jars / two family parcels]
    B --- T[Table under the vine]
    E --- P
```
