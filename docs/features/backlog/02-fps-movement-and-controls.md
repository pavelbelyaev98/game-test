# 02 - FPS movement and controls

Status: task `02` is done. Unity compilation plus 7 EditMode and 14 PlayMode integration checks passed; manual feel review remains.
...
Prefer contextual `LMB Dig`, `Space Jetpack`, and `E Interact` hints or one very short first loop. Do not lock the final tutorial wording before observing a new player. Movement speed, camera sensitivity, reach, cadence, and comfort values are prototype tuning rather than fixed design.
