...
## First-playable task `03a`

- Scope: implement `TerrainVolume` behind `IDigTarget` with one tunable soil material, permanent bedrock/perimeter, local mesh/collider updates and in-memory removal state. Author the [first-playable scene shell](01-core-loop-contract.md) with the existing player, surface, scenery, visible inaccessible water and clear station/return anchors.
...
- Next: make `07a` ready after completion. Disk saving, material variety and production terrain polish remain in task `03` below.

## Full-feature implementation task `03`
