...
The [distinctiveness feedback](../../research/excavation-distinctiveness.md) and [Keep Digging synthesis](../../research/keep-digging-lessons.md) inform readable tool/terrain/discovery presentation. The user wants future excavation guns, with implementation deferred. `154` restores ordinary Scoop, retains the current [silent trial](excavation-modes.md) behind admin opt-in and removes all four sounds. `56`/`120` define meaningful gun progression before `57`/`11` brief its production silhouette; `153` may use gun gameplay once delivered and accepted. The existing trial art remains experimental.
