...
- Preserve the continuous site, untouched start, arbitrary sideways/diagonal cuts, roofs, session state and permanent boundaries. [145](../../development/tasks/145-depth-mineral-progression.md) extends the former 24 × 24 × 12 m volume to 24 × 24 × 32 m at the same 0.125 m resolution, surface position and approved soil/turf art. Old 12 m MainGame saves extend downward with every original sample/pose/identity/value preserved; incompatible unrelated layouts still use recovery. Render and collision share the mesh and local rebuilds.
...
- At 32 m, existing daylight uses a 0.45 ambient minimum so approved minerals remain readable; open sky remains 1 and soil still occludes direct sun. See [lighting](underground-lighting.md); `68` lamp/equipment choices remain paused.
