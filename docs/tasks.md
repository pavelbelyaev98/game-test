...
- [ ] **`014` — Integrate 5 Signature Slice Finds & 3-Tier Reveal** (`docs/concept/05_DISCOVERIES.md`, `14_PROTOTYPE_PLAN.md`): Implement 3-tier reveal treatments: routine commons collect quickly without cleaning chores; distinctive finds (*washing machine, drill, gearbox, mammoth bone, gramophone*) require 50–70% silhouette exposure; uniques/ending parts consume 0 bag slots.
...
- [ ] **`019` — Whole-Object Salvage Tagging & Physical Handling** (`docs/concept/05_DISCOVERIES.md`): Attach recovery tag/clamp to oversized set pieces (vehicles, industrial pumps) claiming them for surface salvage pad transfer. Refine RMB lift/drop and LMB throw with realistic simulation weight.
...
- [ ] **`026` — Balance Battery Drain & Adapt `ReturnWarning`** (`docs/concept/02_CORE_LOOP.md`, `06_PROGRESSION_AND_ECONOMY.md`): Balance battery drain against bag capacity (the holism rule). Adaptive Return Warning acts as an estimate based on depth and ascent energy required.
...
- [ ] **`029` — Implement World Marking & Early Route Lighting** (`docs/concept/03_WORLD_AND_SITE.md`, `04_TOOL_AND_MOVEMENT.md`): Add free world-space markings via the tool (arrow, home, return-here). Introduce basic route lighting at the very first major branch to prevent navigation penalties.
