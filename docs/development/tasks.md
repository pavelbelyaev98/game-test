# Task priority

Top to bottom is priority. Numbers are permanent task IDs; do not sort by ID.
Start a new session from [status.md](status.md), which links the active/next task. Open this list only to review or promote the next eligible task. The [distinctiveness assessment](../research/excavation-distinctiveness.md) explains the current slice-before-bulk-content priority; paused/approval-gated work remains gated.

| ID | Task |
| --- | --- |
| `56` | [Design: equipment tracks and progression milestones](tasks/56-progression-design.md)                 |
| `120` | [Review Shave as a distinctive progression capability](tasks/120-powered-excavation-feel-design.md) |
| `100` | [User playtest: movement and excavation feel](tasks/100-movement-excavation-playtest.md)              |
| `101` | [User playtest: collection and return-trip feel](tasks/101-collection-return-playtest.md)              |
| `117` | [Design: unique-find purpose, frequency and sale versus collection](tasks/117-unique-find-purpose-design.md) |
| `96` | [Design: physics for remaining discovery categories](tasks/96-discovery-physics-design.md)                  |
| `97` | [Design: special-find interaction and treasure chests](tasks/97-special-find-interaction-design.md)    |
| `60` | [Design: the personal discovery display](tasks/60-discovery-display-design.md)                        |
| `57` | [Design: the production presentation brief](tasks/57-presentation-design.md)                          |
| `58` | [Design: the site and material progression](tasks/58-site-and-terrain-design.md)                      |
| `40` | [Design: full discovery roster and asset briefs](tasks/40-discovery-roster.md)                        |
| `155` | [Design reliable first-expedition discovery pacing](tasks/155-first-expedition-discovery-design.md) |
| `106` | [Audit and design: every UI/UX state, visible text and conditional menu](tasks/106-ui-ux-audit-design.md) |
| `118` | [Bottom action bar with backpack icon and current key binding](tasks/118-bottom-action-bar.md)        |
| `93` | [Design: honest return-power advice and warning meaning](tasks/93-return-warning-design.md)             |
| `81` | [Design: brightness and reticle accessibility](tasks/81-visual-accessibility-design.md)               |
| `08` | [Production visual and audio foundation](tasks/08-production-presentation.md)                         |
| `82` | [Brightness and reticle accessibility settings](tasks/82-visual-accessibility.md)                     |
| `05` | [Finish production FPS controls and HUD acceptance](tasks/05-fps-controls.md)                         |
| `06` | [Finish production excavation presentation acceptance](tasks/06-excavation-presentation.md)           |
| `07` | [Finish production inventory inspection acceptance](tasks/07-inventory-inspection.md)                 |
| `09` | [Production starter finds and collection presentation](tasks/09-starter-discoveries.md)               |
| `25` | [Independent shovel speed and strength purchases](tasks/25-independent-shovel-upgrades.md)            |
| `11` | [Visible evolving shovel and digging response](tasks/11-visible-shovel-progression.md)                |
| `94` | [Update return warning logic and presentation](tasks/94-return-warning-update.md)                       |
| `15` | [Integrate and validate one complete production trip](tasks/15-complete-trip.md)                      |
| `39` | [Terrain materials and upgrade-sensitive resistance](tasks/39-terrain-materials.md)                   |
| `42` | [First recognizable discovery and scale batch](tasks/42-grounded-discoveries.md) |
| `10` | [Passive detector feedback for non-minor discoveries](tasks/10-passive-detector.md)                   |
| `51` | [Persistent personal discovery snapshots and display](tasks/51-discovery-display.md)                  |
| `102` | [User playtest: production discovery and feedback](tasks/102-production-discovery-playtest.md)        |
| `148` | [Validate the excavation and discovery slice before bulk content](tasks/148-excavation-discovery-slice-validation.md) |
| `153` | [Test excavation positioning with honest silent clips](tasks/153-excavation-hook-positioning.md) |
| `149` | [Design rare uncanny variants of familiar finds](tasks/149-uncanny-find-variants-design.md) |
| `150` | [Integrate a selected uncanny variant batch](tasks/150-uncanny-find-variants.md) |
| `45` | [Weighted discovery pools and related-item clusters](tasks/45-discovery-generation.md)                |
| `55` | [Design: achievements and completion goals](tasks/55-achievement-design.md)                           |
| `41` | [Complete production ordinary discovery roster](tasks/41-ordinary-discoveries.md)                     |
| `43` | [Middle distinctive discovery batch](tasks/43-middle-discoveries.md)                                  |
| `44` | [Late distinctive discovery batch](tasks/44-late-discoveries.md)                                      |
| `70` | [Design: early HOME and revisit-marker comparison](tasks/70-navigation-and-revisit-research.md)       |
| `99` | [Design: return mobility and a placed teleporter](tasks/99-return-mobility-design.md)                  |
| `47` | [Paid jetpack power and efficiency progression](tasks/47-jetpack-upgrades.md)                         |
| `84` | [Design: late and postgame discovery assistance](tasks/84-completion-assistance-design.md)            |
| `49` | [Paid detector progression](tasks/49-detector-upgrades.md)                                            |
| `119` | [Design: richer backpack inspection and dropping stored finds](tasks/119-backpack-inspection-release-design.md) |
| `83` | [Distinctive-find inspection flavor](tasks/83-discovery-inspection-flavor.md)                         |
| `71` | [Research: Optional C4 and forgiving placement](tasks/71-dynamite-design-research.md)                 |
| `59` | [Design: Decide return pressure and fall consequences](tasks/59-return-and-fall-design.md)            |
| `50` | [Fall consequences and balanced rescue](tasks/50-fall-consequences.md)                                |
| `103` | [User playtest: equipment and expedition choices](tasks/103-equipment-expedition-playtest.md)          |
| `61` | [Design: the final discovery and ending payoff](tasks/61-ending-design.md)                            |
| `52` | [Mystery progression, final discovery and same-save Continue](tasks/52-ending-and-continue.md)        |
| `85` | [Conditional discovery completion assistance](tasks/85-completion-assistance.md)                      |
| `62` | [Design: Choose the rare permanent rewards](tasks/62-passive-reward-design.md)                        |
| `36` | [Add 2–4 very rare buried permanent passive upgrades](tasks/36-buried-passive-upgrades.md)            |
| `104` | [User playtest: full adventure and Continue](tasks/104-full-adventure-playtest.md)                      |
| `37` | [Balance discovery and useful purchases through the 2–3 hour run](tasks/37-full-run-pacing.md)        |
| `53` | [Same-save Steam achievements](tasks/53-steam-achievements.md)                                        |
| `137` | [Review one detailed reservoir section](tasks/137-reservoir-first-section-review.md) |
| `130` | [Integrate user-approved reservoir sections](tasks/130-clouds-and-drained-reservoir.md) |
| `68` | [Design: Passive underground lighting](tasks/68-underground-lighting-design.md)                       |
| `69` | [Integrate passive underground lighting](tasks/69-underground-lighting.md)                            |
| `80` | [Incremental terrain capture and non-blocking save budgets](tasks/80-incremental-terrain-saving.md)   |
| `91` | [Design: save portability and Steam Cloud](tasks/91-save-portability-design.md)                       |
| `22` | [Verify release builds exclude developer admin access](tasks/22-release-admin-exclusion.md)           |
| `54` | [Full-game performance, comfort and Windows release qualification](tasks/54-release-qualification.md) |
| `147` | [Experimental excavation modes — production adoption deferred](tasks/147-expressive-excavation-modes.md) |
| `151` | [Optional design: excavation timelapse](tasks/151-excavation-timelapse-design.md) |
| `152` | [Optional design: automatic camp evolution](tasks/152-automatic-camp-evolution-design.md) |

Task files own status, prerequisites, research, questions and acceptance. Remove finished rows; retain their [completion records](completed/) and numbered specs.
Cancelled: [76 — Windows Sandbox review](tasks/76-isolated-windows-review.md). Toggle digging is implemented in [78](completed/78-input-accessibility.md). Next new task ID: `156`.
