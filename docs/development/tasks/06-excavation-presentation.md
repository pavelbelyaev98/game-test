...
- Verified mechanic: `TerrainVolume` behind `IDigTarget`, one soil response, permanent bedrock/perimeter, local mesh/collider updates and in-memory removal state in the [main game scene](../../features/core-gameplay.md). Retain the existing player, surface/return anchors and user-owned terrain work.
