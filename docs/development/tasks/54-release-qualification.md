...
Feature: [release validation](../../features/release.md). [Queue](../tasks.md). [Research basis](../../research/player-review-findings.md).
...
IT IS OF EXTREME IMPORANT TO MAKE THE GAME HARD TO REVERSE ENGINEER, so research IL2CPP, obfuscation etc....
...
- Qualify against the Windows support matrix and frame-time/load/save/memory budgets accepted in 63. Bring back only evidence-backed changes to those targets or remaining presentation scope; actual additional assets/audio still need explicit approval.
