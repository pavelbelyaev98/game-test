...
- Consume [89's starter item selection, counts and specifications](89-starter-minor-find-design.md) for `09`; item selection is no longer owned here. Brief their shared presentation, visible shovel milestones for `11`, and initial detector feedback for `10`. Preserve each starter item's physical eligibility independently from value/rarity, plus single-signal focus/quiet gaps. The remaining full roster belongs to `40`.
