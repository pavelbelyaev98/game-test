...
Feature: [presentation](../../features/presentation.md). Implementation: `08`–`11`. [Queue](../tasks.md).

Retain [74/75's shared Toolkit menu and HUD direction](../../features/core-gameplay.md) and [authoring sources](../ui-authoring.md). `08` refines the existing presentation; the HUD framework migration is complete. The retained EventSystem is input infrastructure, not a second presentation system.
...
- The event matrix, surface-payoff examples and affordance sheet resolve presentation choices concretely, with intended feeling and failure cases from the risk register. Retain omitted/rejected alternatives and their reasons; a new feedback queue or recognition delay is not preselected.
...
- Earlier scrape/thud, powered impacts, motor/torque and detector-volume proposals from the [report](../../research/player-review-findings.md) may now be reviewed under the selected audio direction; `10`/`49` still need sparse, readable cues that work with sound muted.
