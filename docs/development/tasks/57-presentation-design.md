...
Type: design/research; documentation only. Status: `planned`. Prerequisites: `56`; respect the user-deferred terrain/art scope.
...
- Propose a bounded visual brief using descriptions and existing reference links: setting direction, readability/scale, diggable ground versus boundaries, camera/tool visibility, HUD hierarchy and the terrain ownership/handoff boundary. Contrast eventual diggable hard stone with true finite-world boundaries by structure/scale/silhouette, not two identically styled rocks with different permissions. Preserve the already-visible inventory/battery facts and clear sell/upgrade/save meaning. Respect `64`/`65` comfort decisions; visible powered-tool feedback must not depend on compulsory camera motion.
- List named starter finds for `09`, visible shovel milestone briefs for `11`, and the initial detector feedback proposal for `10`. Define starter physical eligibility and any small noteworthy exception separately from sale value/rarity, plus the single-signal focus/quiet-gap behaviour. The full discovery roster remains `40`; do not duplicate its content planning.
...
- Consume `68`'s player-light contract and prepare `69` integration ownership. Consider visible major battery/detector/jetpack hardware only where players can meaningfully see it; review whether any such additions earn their scope before assigning them to `46`/`47`/`49`. Shovel milestone visuals/audio remain required; every minor stat tier need not get a new model.
...
Review the proposed direction, terrain handoff and exact production scope; select the starter objects, boundary language, equipment appearance and initial feedback approach. Discuss which non-shovel equipment milestones deserve visible hardware, from which view, and why; a reference or asset brief is not approval. Ask only for the relevant decisions, with concrete examples. Each actual new asset/audio batch still needs separate explicit approval before creation/import under the existing rules.
