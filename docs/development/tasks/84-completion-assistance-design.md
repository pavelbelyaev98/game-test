# Task 84 - Design late/postgame discovery assistance
...
- Deliver a concrete option comparison and recommend one with practical search evidence or clearly labelled representative scenarios. Review whether completion filtering is acceptable, when it unlocks and what counts as documented.
- Record the selected exception or retained exclusion in the detector feature; `49` consumes that decision. Specify final ending-state integration with `61`/`52` before delivery.
- If included, finalize `85` scope/prerequisites; if omitted, retire `85` and remove dependencies. Research completion never claims the aid is playable.
