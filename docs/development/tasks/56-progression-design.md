...
- Preserve money-based sequential purchases, powerful old-obstacle revisits, click-and-hold input and the current chosen scoop feel. The user explicitly wants future excavation guns, with implementation deferred. Include their meaningful capabilities in the proposal without assuming the [four-mode trial](../../features/backlog/excavation-modes.md) or uniform tier scaling settles progression. Shave is liked and the current normal digging remains the baseline. Propose qualitative milestones first; `120` then resolves Shave adoption without creating a prerequisite cycle. Do not re-ask whether guns are wanted.
