...
Type: design/research. Status: `ready`; the user has returned to progression design and requests better, more distinctive capabilities. Previously selected backpack/shared-fuel choices remain implemented. Prerequisites: `35`, `63` (complete). This task is next after `154`; no new purchases or broad equipment implementation are selected yet.

Features: [shovel](../../features/progression.md), [equipment economy](../../features/progression.md). Implementation: `25`, `11`, `39`, `46`–`49`. [Queue](../tasks.md).
...
- Consume [both Keep Digging reports and the user verdict](../../research/keep-digging-lessons.md). Produce two coherent small progression packages, each with two or three major capability changes, explicit early/middle/late situations, same-formation/route before-after examples, first useful application and a remaining reason to explore. Explain what the player can do differently without stat labels; retain useful incremental tiers only where they earn a place. Predictable money goals must work without random recipes, optional rare finds or required max-all purchases.
...
- Consume selected refill/travel policies from `98`/`99`, or retain the implemented baseline if they are omitted/deferred. Recheck practical value and necessary costs; the user has resumed this design comparison, but extra implemented tracks still require its reviewed artifact.
...
- Preserve money-based sequential purchases, powerful old-obstacle revisits, click-and-hold input and the current chosen scoop feel. The user explicitly wants future excavation guns, with implementation deferred. Include their meaningful capabilities in the proposal without assuming the [four-mode trial](../../features/excavation.md) or uniform tier scaling settles progression. Shave is liked and the current normal digging remains the baseline. Propose qualitative milestones first; `120` then resolves Shave adoption without creating a prerequisite cycle. Do not re-ask whether guns are wanted.
...
- Apply the [distinctiveness hypothesis](../../research/excavation-distinctiveness.md): each major milestone should change useful excavation shape, information, old-material access or scale. Smaller levels may stay numerical. Supply a same-formation before/after example and player explanation without stats for `103`/`148`; do not merely rename speed bonuses.
...
- Use the [comparator synthesis](../../research/player-review-findings.md) to map milestones across the whole run: basic, reinforced, powered and absurd versions of the same shovel. New material cannot require a weak replacement tool or cancel all prior purchases.
