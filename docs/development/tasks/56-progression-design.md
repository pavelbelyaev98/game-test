...
- Preserve money-based sequential purchases, powerful old-obstacle revisits, click-and-hold input and the current chosen scoop feel. The user now rejects treating the [four-mode trial](../../features/backlog/excavation-modes.md) as the game: Shave is liked, but the existing digging is preferred and uniform tier scaling is insufficient progression. Propose the qualitative milestones first; `120` then resolves Shave adoption without creating a prerequisite cycle.
