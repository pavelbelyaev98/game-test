...
- Retain the existing always-visible carried count/capacity and battery HUD when replacing inspection presentation. Capacity visibility is already implemented in `FpsHud`; do not add a duplicate indicator task or turn it into a discovery-completion counter.
